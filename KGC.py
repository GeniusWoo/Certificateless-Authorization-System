...
class KGC:
    # KGC类：密钥生成中心类，包含KGC的IDi，Ti，ti，P_pub，oP_pub等信息以及加解密数据处理
    # P, o, d, oP_pub, ooP_pub, Ti, ti, IDi, 
    def __init__(self, nameId, oP_pub):
        self.nameId = nameId
    ...
    def setSecretValue(self):
        ...
        print("Ti :" , self.Ti)
        print("IDi for KGC is ", self.IDi)

    def setPrivateKey(self,Ri, ki):
        print("\n\n********* KGC ***********")
        P, oP_pub, Ti, ti, IDi = self.P, self.oP_pub, self.Ti, self.ti, self.IDi

        lhs = P.pt_mul(ki)

        term1 = Ri
        term2 = oP_pub.pt_mul(H7(Ri,Ti,IDi))
        term3_1 = oP_pub.pt_mul(ti)
        term3 = P.pt_mul(H3(term3_1,IDi))

        rhs1 = term1.pt_add(term2)
        rhs = rhs1.pt_add(term3)

        si = ki- H3(oP_pub.pt_mul(ti), IDi)

        print("\nKGC Secret Key - si: " + str(si) + ", ti: "+ str(ti))
        print("KGC Public Key - Ri: " + str(Ri) + ", Ti: "+ str(Ti))
        ...
        print(lhs,rhs)	
        if lhs == rhs:
            print("Partial PrivateKey Verification Successful")
            return True
        else:
            print("Partial PrivateKey Verifcation Unsuccessful")
            return False
		# Sender

    def encrypt(self,m):
        P, oP_pub, Ts, ts, IDs, Rs, ss = self.P, self.oP_pub, self.Ti, self.ti, self.IDi, self.Ri, self.si
        ...
        t21 = oP_pub.pt_mul(H7(Rs,Ts,IDs))
        t2 = Rs.pt_add(t21)
        t2 = t2.pt_add(Ts)
    ...
    def rekeygen(self, Rj_list, Tj_list, IDj_list):
        alpha, z, w, ss, ts, G, oP_pub = self.alpha, self.z, self.w, self.si, self.ti,self.G, self.oP_pub
        ...
        q = EcGroup(self.nameId).parameters()["p"]
    ...
    def reencryption(self, CT,rk1, rk2):
        print("\nReencryption done\n")
        C31 = CT[0].pt_mul(rk1)
        C41 = rk2
        return C31,C41
    ...
    def gen_CT(self):
        verify_num=random.randrange(1000000000, 9999999999, 1)
        CT=self.encrypt(str(verify_num))
        print("Encrypted message : ",CT)
        return CT,verify_num

    def printf(self):
        print("P :", self.P)
        print("self. :", self.d)
        print("oP_pub :", self.oP_pub)
